...
(define-extended-language λF-ACC λF-ANF
  (τ σ ::= .... ;; Types
     (tcode (α ...) (τ ...) β σ)  ;; ∀ (α ...). τ → ... → ∀ α. τ
     (vcode (α ...) (τ ...) σ σ)) ;; ∀ (α ...). τ → ... → τ → τ
...
(module+ test
  (redex-chk
   ...
   #:eq (tcode (a b) (b a) c (→ a c))             (tcode (i j) (j i) k (→ i k))
   #:eq (vcode (a b) (b a) (→ a b) (→ b a))       (vcode (i j) (j i) (→ i j) (→ j i))
...
(define-extended-judgement-form λF-ACC F.⊢τ
  ...
  [(where Δ_0 (Δ+ Δ α ...))
   ...
   (⊢τ Δ_0 σ_2)
   --------------------------------------- "τ-vcode"
   (⊢τ Δ (vcode (α ...) (τ ...) σ_1 σ_2))])

(module+ test
  (redex-judgement-holds-chk
   (⊢τ ·)
   [(tcode (a b) (b a) c (→* a b c))]
   [(vcode (a b) (b a) (→ a b) (→ b a))]))
...
(define-judgement-form λF-ACC
  ...
  [(where Δ_0 (Δ* α ...))
   ...
   (⊢e (Δ_0 β) (Γ* (x : τ) ...) e σ)
   --------------------------------------------------------------- "tcode"
   (⊢k (Λ (α ...) ([x : τ] ...) β e) (tcode (α ...) (τ ...) β σ))]

  [(where Δ_0 (Δ* α ...))
   (⊢τ Δ_0 τ) ...
   (⊢e Δ_0 (Γ* (x : τ) ... (y : σ_1)) e σ_2)
   --------------------------------------------------------------------------- "vcode"
   (⊢k (λ (α ...) ([x : τ] ...) (y : σ_1) e) (vcode (α ...) (τ ...) σ_1 σ_2))])

(module+ test
  (redex-judgement-holds-chk
   (⊢k)
   [(Λ (a b) ([x : a] [y : (∀ b b)]) c (y [c])) (tcode (α_1 β_1) (α_1 (∀ β_2 β_2)) α_2 α_2)]
   [(λ (a b) ([x : a] [y : (→ a b)]) (z : a) (y z)) (vcode (α β) (α (→ α β)) α β)]))
